...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'note',
          tone: 'gotcha',
          title: 'Set an attribute and it can vanish within a tick',
          runs: [
            { code: 'show' }, ', ', { code: 'showimage' }, ' and the asset commands claim the display; ',
            { code: 'bgcolor' }, ', ', { code: 'textcolor' }, ', ', { code: 'textpos' }, ', ',
            { code: 'textstyle' }, ' and ', { code: 'cls' },
            ' do not. With the display unclaimed the board keeps repainting the main page on its own schedule, ',
            'and that repaint re-rolls a random background — so ', { code: 'bgcolor 160 wait 1000 cls' },
            ' visibly loses its blue. Claim the display with a ', { code: 'show' },
            ' first and everything after it sticks.',
          ],
        },
        {
          type: 'prose',
          runs: [
            { b: 'The test build changes what these commands draw.' },
            ' The firmware branch ', { code: 'feature/expose-logovm-text-commands-hid' }, ' (build ',
            { code: 'v4.0.0-hidtext' },
            ') adds nothing to the language, but the same program renders differently. All of these are ',
            'fixes; ', { code: 'cls' }, ' is the one to watch, because a program that sets a background and then ',
            'calls it expecting white now keeps its own colour.',
          ],
        },
        {
          type: 'table',
          head: ['Command', 'Released firmware', 'Test build'],
          mono: [0],
          rows: [
            ...
            ['bgcolor, textcolor, textpos, textstyle', 'the main page can repaint over them, per the note above', 'claim the display, so they take effect and hold'],
